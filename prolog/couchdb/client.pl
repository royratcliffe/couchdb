...
Client.welcome() := Welcome :-
    Client.get_url('') = Welcome,
    tag_dict(Welcome, couchdb_welcome).

Client.all_dbs() := AllDbs :-
    Client.get_url('_all_dbs') = AllDbs0,
...
Client.get_url(URL) := Reply :-
    http_get(Client.url(URL), Reply, [json_object(dict)]).

Client.put_url(URL, Data) := Reply :-
    http_put(Client.url(URL), json(Data), Reply, [json_object(dict)]).
